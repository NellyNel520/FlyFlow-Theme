...
- 2026-02-18: Expanded PDP tab content architecture with premium editable defaults per tab (overview/highlights/spec grid/care-fit, size-guide helper + fit notes, reviews empty-state + FAQ blocks, shipping/returns policy-priority cards), including new `tab_spec` and `reviews_faq` blocks and real-data metafield/policy fallback logic.
